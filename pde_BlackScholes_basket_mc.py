...
def train(T,
    ...
    fbsde.eval()
    discounted_payoff = fbsde.unbiased_price_mc(ts=ts, x0=x0, option=option, MC_samples=50000)
    discounted_payoff_antithetic = fbsde.unbiased_price_mc(ts=ts, x0=x0, option=option, MC_samples=25000, antithetic=True)
...
if __name__ == "__main__":
    ...
    for i in range(args.n_seeds):
        seed = args.seed + i
        set_seed(seed)
        results_path = os.path.join(args.base_dir, "BS", "basket_{}".format(args.d), "antithetic", "seed{}".format(seed))
